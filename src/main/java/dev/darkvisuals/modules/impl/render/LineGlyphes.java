...
public class LineGlyphes extends Module {
    ...
    public LineGlyphes() {
        super("LineGlyphes", Category.Render, "Анимированные линии в 3D пространстве");
    ...
    public void onUpdate(EventPlayerTick event) {
        ...
        for (Path path : paths) {
            path.update(speed.getValue());
        }

        if (paths.size() < count.getValue()) {
    ...
    public void onRender3D(EventRender3D.Game event) {
        if (paths.isEmpty() || fullNullCheck()) return;
    ...
    private void renderPaths(MatrixStack matrices, Color color, Vec3d cameraPos, float partialTicks, boolean isGlow, float widthMul, float alphaMul) {
        Matrix4f model = matrices.peek().getPositionMatrix();
        Tessellator tessellator = Tessellator.getInstance();
        BufferBuilder buffer = tessellator.begin(VertexFormat.DrawMode.LINES, VertexFormats.LINES);

        float baseWidth = thickness.getValue();
        ...
        for (Path path : paths) {
            ...
            for (int i = 0; i < points.size() - 1; i++) {
                ...
                int b = color.getBlue();
                int a = MathHelper.clamp((int) (segmentAlpha * 255), 0, 255);
    ...
    private static class Path {
        ...
        private final Animation alphaAnim = new Animation(1200, 1.0, true, Easing.BOTH_SINE);
