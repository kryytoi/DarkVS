...
public class Predictions extends Module implements ThemeManager.ThemeChangeListener {
    ...
    public Predictions() {
        ...
        getSettings().add(showPearl);
        getSettings().add(showBow);
        getSettings().add(showCrossbow);
        getSettings().add(showTrident);
        getSettings().add(showWhenHolding);
        getSettings().add(showLandingInfo);
        getSettings().add(highlightPlayers);
    ...
    private void renderProjectileTrajectory3D(EventRender3D.Game e, boolean hasFlyingProjectiles) {
        ...
        // геометрия просто ставится в общую очередь — миксин кадра сам подготовит и нарисует её
